...
**Resolution (2026-06-18):** Superseded by gabadi/swarm-forge#51 — decision tree outcome is "accept (PR doc only)"; no source annotation. Commit f1b65d1 already documents it in the commit message. No source action needed.
...
**Resolution (2026-06-18):** Fixed — `--feature-path` CLI arg added to `generate_entrypoint.py`; falls back to deriving from IR filename. `run-acceptance` updated to pass `--feature-path "$FEATURE"` explicitly per loop iteration.

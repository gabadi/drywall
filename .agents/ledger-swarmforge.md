...
2026-06-18 | (architect) | architect | convention-gap | rejected→swarmforge | applied | Before writing handoff commit, derive with git rev-parse --short=10 HEAD not git log --oneline
...
2026-06-18 | (hardender) | hardender | convention-gap | rejected→swarmforge | applied | handoff role names case-sensitive; use git rev-parse --short=10 for commit abbrev
...
2026-06-18 | (integrator-merge) | integrator | wrong-path | rejected→swarmforge | applied | Before retrying gh pr merge after error, run gh pr view --json state to check if already merged
...
2026-06-19 | (cleaner-clean) | cleaner | convention-gap | rejected→swarmforge | applied | Always run git rev-parse --short=10 HEAD immediately before writing git_handoff draft
...
2026-06-19 | (specifier-exclusions) | specifier | convention-gap | rejected→swarmforge | applied | git_handoff commit: abbrev via git rev-parse --short=10 HEAD (10 chars) not git log default
...
2026-06-19 | (js-ts-spec) | specifier | convention-gap | rejected→swarmforge | applied | specifier role: git reset --hard origin/default gated by auto-classifier; add allow-rule or startup note
...
2026-06-21 | ca7ffb05-6213-4c90-bd00-6c83407f5c79 | hardender | wrong-path | rejected→swarmforge | applied | swarm_handoff.sh: requires git rev-parse --short=10; role names case-sensitive (check swarmforge.conf)
...
2026-06-21 | (cli-surface-qa-verif) | QA | convention-gap | rejected→swarmforge | applied | commit abbrev: always use git rev-parse --short=10 HEAD rather than git log --oneline
...
2026-06-21 | b4761e12-ce6a-4176-88b1-82541ca6fd67 | architect | convention-gap | rejected→swarmforge | applied | git_handoff commit: always use git rev-parse --short=10 HEAD (10 chars); default --short=7 is wrong
...
2026-06-21 | cd1fedb9-ccf8-4ea9-80a9-f688b5733499 | specifier | convention-gap | rejected→swarmforge | applied | Investigate: specifier git reset --hard origin/<default> routinely denied by sandbox; add guarded form or allow-rule
...
2026-06-21 | 8fbf47f7-c991-464e-8232-c9d7b6c7debb | ux-engineer | convention-gap | rejected→swarmforge | applied | Clarify in ux-engineer role: `# UX INTENT: none` comment satisfies "no ## UX Intent section" — pass through to cleaner
...
2026-06-21 | 2308d3c6-531d-460c-8ab1-ca4d79eae2ad | ux-engineer | convention-gap | rejected→swarmforge | applied | swarm-persona: if persona content already in last user message from /swarm-persona command, skip Skill tool invocation
...
2026-06-21 | 4f568388-4756-468f-b118-65f5ed8d4f6f | integrator | convention-gap | rejected→swarmforge | applied | Remove --timeout 1800 from gh pr checks --watch in integrator role — flag does not exist in installed gh CLI
2026-06-21 | 4f568388-4756-468f-b118-65f5ed8d4f6f | integrator | wrong-path | rejected→swarmforge | applied | Before gh pr merge --delete-branch, check whether any worktree holds feature branch; omit --delete-branch if so
